...
export const createNote = {
    ...
    initNote: () => {
        document.addEventListener('mousedown', e => {
            if (!createData.enabled.shape)
                return;
            if (createNote.checkBoundary(e))
                return;
            if (activeElement)
                return;
            createData.firstPos = {
        ...
        document.addEventListener('mousemove', (e) => {
            ...
            if (!activeElement || activeElement instanceof Line)
                return;
            if ((activeElement instanceof Textbox || activeElement instanceof Rect) &&
            ...
            if (createData.param.shape.constraint) {
                const size = Math.round((Math.abs(e.pageX - createData.firstPos.x) + Math.abs(e.pageY - createData.firstPos.y)) / 2) * 2;
    ...
    initPolyline: () => {
        document.addEventListener('click', (e) => {
            if (!createData.enabled.polyline)
                return;
            if (createNote.checkBoundary(e))
                return;
            if (activeElement && activeElement instanceof Line) {
    ...
    init: () => {
        ...
        document.addEventListener('mousemove', () => {
            if (!Object.values(createData.enabled).includes(true))
                return;
