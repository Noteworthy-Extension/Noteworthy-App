export const NoteHistory = {
    ...
    pushEvent: (event) => {
        if (NoteHistory.events.length > NoteHistory.eventIndex) {
            NoteHistory.events.splice(NoteHistory.eventIndex, NoteHistory.events.length - NoteHistory.eventIndex);
        }
        NoteHistory.events.push(event);
        NoteHistory.eventIndex++;
    },
    undo: () => {
        if (NoteHistory.eventIndex <= 0)
            return;
        NoteHistory.eventIndex--;
        NoteHistory.events[NoteHistory.eventIndex].undo();
    },
    redo: () => {
        if (NoteHistory.eventIndex >= NoteHistory.events.length)
            return;
