export const dropdownSelect = {
	init: (label: string, value: string | number, fn: (newVal: string) => void) => {
		const activeItem = document.querySelector(
			`#${label}.dropdown_select #item-${value.toString()}.dropdown_select-item`
		);
		if (activeItem) activeItem.classList.add('active');

		(<NodeListOf<HTMLElement>>(
			document.querySelectorAll(`#${label}.dropdown_select .dropdown_select-item`)
		)).forEach(element => {
			element.addEventListener('click', () => {
				document
					.querySelectorAll(`#${label}.dropdown_select .dropdown_select-item`)
					.forEach(element => element.classList.remove('active'));
				element.classList.add('active');

				fn(element.id.replace('item-', ''));
			});
		});

		(<HTMLElement>document.querySelector(`#${label}.open_dropdown_select`)).addEventListener(
			'click',
			() => {
				document.querySelector(`#${label}.open_dropdown_select`)?.classList.toggle('active');
				(<HTMLElement>document.querySelector(`#${label}.dropdown_select`)).classList.toggle('open');
			}
		);

		const eventFn = (e: MouseEvent) => {
			if (!document.querySelector(`#${label}.dropdown_select`)) return;
			if (
				(<HTMLElement>e.target).closest(`#${label}.dropdown_select`) ||
				(<HTMLElement>e.target).closest(`#${label}.open_dropdown_select`)
			)
				return;
			(<HTMLElement>document.querySelector(`#${label}.open_dropdown_select`)).classList.remove(
				'active'
			);
			(<HTMLElement>document.querySelector(`#${label}.dropdown_select`)).classList.remove('open');
		};
		document.removeEventListener('click', eventFn);
		document.addEventListener('click', eventFn);
	},

	html: (label: string, _value: string | number) => {
		const list: Record<string, string> = dropdownSelect.list[label];
		return `
			<div class="open_dropdown_select editbar_button Noteworthy_tooltip" id="${label}">
			<span class="tool_tip_text">${label.split('-').join(' ')}</span>
				${dropdownSelect.icon[label]}
			</div>
            <div class="dropdown_select" id="${label}">
				<div class="dropdown_select-list">
						${Object.keys(list)
							.map(key => {
								return `<div class="dropdown_select-item" id="item-${key}">${list[key]}</div>`;
							})
							.join('')}
				</div>
            </div>
        `;
	},

	list: {
		'line-Start': {
			none:
				'<img src="data:image/png;base64," />',
			arrow:
				'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><path d="M 50 0 L 100 100 L 0 100 Z" fill="none" stroke="black" stroke-width="6" /></svg>',
			arrow2:
				'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><path d="M 0 0 L 100 0 L 50 100 Z" fill="#000" stroke="black" stroke-width="6" /></svg>',
			dot: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><circle cx="50" cy="50" r="50" fill="none" stroke="black" stroke-width="6" /></svg>',
			dot2:
				'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><circle cx="50" cy="50" r="50" fill="#000" stroke="black" stroke-width="6" /></svg>',
		},
		'line-End': {
			none:
				'<img src="data:image/png;base64," />',
			arrow:
				'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><path d="M 50 0 L 100 100 L 0 100 Z" fill="none" stroke="black" stroke-width="6" /></svg>',
			arrow2:
				'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><path d="M 0 0 L 100 0 L 50 100 Z" fill="#000" stroke="black" stroke-width="6" /></svg>',
			dot: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><circle cx="50" cy="50" r="50" fill="none" stroke="black" stroke-width="6" /></svg>',
			dot2:
				'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><circle cx="50" cy="50" r="50" fill="#000" stroke="black" stroke-width="6" /></svg>',
		},
		pattern: {
			//stroke-dasharray with different lines
			none: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 10"><line x1="0" y1="0" x2="200" y2="0" stroke="black" stroke-width="4" /></svg>`,
			'5_5': `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 10"><line x1="0" y1="0" x2="200" y2="0" stroke="black" stroke-width="4" stroke-dasharray="5,5" /></svg>`,
			'10_10': `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 10"><line x1="0" y1="0" x2="200" y2="0" stroke="black" stroke-width="4" stroke-dasharray="10,10" /></svg>`,
			'20_20': `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 10"><line x1="0" y1="0" x2="200" y2="0" stroke="black" stroke-width="4" stroke-dasharray="20,20" /></svg>`,
		},
		width: {
			'1': '1px',
			'2': '2px',
			'3': '3px',
			'4': '4px',
			'6': '6px',
			'8': '8px',
			'12': '12px',
			'16': '16px',
			'24': '24px',
		},
		opacity: {
			'0%': '0',
			'25%': '25%',
			'50%': '50%',
			'75%': '75%',
			'100%': '100%',
		},
		radius: {
			'0': 'none',
			'3': '3px',
			'5': '5px',
			'10': '10px',
			'15': '15px',
		},
	},

	icon: {
		width: `<img src="data:image/png;base64,"/>`,
		opacity: `<img src="data:image/png;base64,">`,
		radius: `<img src="data:image/png;base64," />`,
		pattern: `<img src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAB4AAAAeCAYAAAA7MK6iAAAABmJLR0QA/wD/AP+gvaeTAAAAM0lEQVRIie3PsQkAIBADwOD+WznYO4GIxYvFHaRLkSQA8KOZpDaZXd1xGFUXB7q6APDIAijSGYCwrg2gAAAAAElFTkSuQmCC" />`,
		'line-Start': `<img src="data:image/png;base64," />`,
		'line-End': `<img src="data:image/png;base64," />`,
	},
};
